  ...
  switch (gameState){
    ...
    case GAME_RUN:
        ...
        // -------------------------------
         ...
         // car1 hitTest
         if (abs(leftCar1CX - frogCX) <= 32 && 144 == frogCY ){
             image(imgDeadFrog, frogX, frogY);
             life--;
             gameState = FROG_DIE;
        }        
         // car2 hitTest
         if (abs(leftCar2CX - frogCX) <= 32 && 272 == frogCY ){
             image(imgDeadFrog, frogX, frogY);
             life--;
             gameState = FROG_DIE;
        }        
         // car3 hitTest
         if (abs(rightCar1CX - frogCX) <= 32 && 208 == frogCY ){
             image(imgDeadFrog, frogX, frogY);
             life--;
             gameState = FROG_DIE;
        }        
         // car4 hitTest
         if (abs(rightCar2CX - frogCX) <= 32 && 336 == frogCY ){
             image(imgDeadFrog, frogX, frogY);
             life--;
             gameState = FROG_DIE;
